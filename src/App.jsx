...
function PublicMobileNavbar({ scrolled }) {
  ...
  return (
    <div style={{
      ...
      {/* Right side — collapses when scrolled */}
      <div style={{
        marginLeft: "auto",
        ...
        {!isLoggedIn && (
          <>
            <button onClick={() => setScreen("login")}
              style={{ padding: "7px 12px", borderRadius: "8px", border: "1px solid var(--border)", background: "transparent", color: "var(--text-primary)", fontSize: "12px", fontWeight: 600, cursor: "pointer", fontFamily: FONT, whiteSpace: "nowrap" }}>
              Log in
            </button>
            <button onClick={() => setScreen("signup")}
              style={{ padding: "7px 12px", borderRadius: "8px", border: "none", background: `linear-gradient(135deg,${BRAND},#EA6C0A)`, color: "#fff", fontSize: "12px", fontWeight: 700, cursor: "pointer", fontFamily: FONT, whiteSpace: "nowrap", boxShadow: `0 2px 8px ${BRAND}40` }}>
              Sign up
            </button>
          </>
        )}
...
// ── Public mobile shell (unauthenticated) ─────────────────────
...
export default function App() {
  ...
  if (desktop && !isLoggedIn) return (
    <div style={{ height:"100%", overflow:"hidden", display:"flex", flexDirection:"column" }}>
      {screen === "pendingEvent"
        ? <div style={{ flex:1, minHeight:0, overflowY:"auto" }}><PublicEventPage /></div>
        : <div style={{ flex:1, minHeight:0, overflowY:"auto" }}><AttendeeHome /></div>
      }
      <Toaster position="top-right" />
      <CookieBanner />
    </div>
