...
import {
  ...
  PlusCircle, Zap, ScanLine, Search, Activity
} from "lucide-react";
      ...
      {role === "organizer" && (
        <motion.div whileTap={{ scale: 0.9 }}
          ...
          style={{
            ...
            cursor: "pointer", boxShadow: "0 4px 20px rgba(245,166,35,0.5)", zIndex: 200,
          }}>
...
function NavItem({ icon: Icon, label, active, collapsed, onClick, title }) {
  return (
    <motion.div whileTap={{ scale: 0.94 }} onClick={onClick} title={title}
      ...
      onMouseLeave={e => { e.currentTarget.style.background = active ? "rgba(245,166,35,0.1)" : "transparent"; }}>
...
// ── Desktop topbar — NO user avatar (lives in sidebar only) ──
    ...
    <div style={{
      ...
      <div style={{ flex: 1, maxWidth: "420px", position: "relative" }}>
        ...
        <input value={searchQ}
          ...
          onFocus={e => { e.target.style.borderColor = "var(--brand)"; e.target.style.background = "var(--bg-card)"; e.target.style.boxShadow = "0 0 0 3px var(--brand-glow)"; }}
        ...
        {searchQ && (
          <div onClick={() => setSearchQ("")}
            style={{ position: "absolute", right: "11px", top: "50%", transform: "translateY(-50%)", cursor: "pointer", color: "var(--text-muted)", fontSize: "13px" }}>✕</div>
        )}
      </div>

      {/* Right actions — NO avatar here */}
      <div style={{ display: "flex", alignItems: "center", gap: "10px", flexShrink: 0 }}>

        {/* Polygon live indicator */}
        <div style={{
          ...
          />
          <span style={{ fontSize: "10px", fontWeight: 700, color: "#16a34a", fontFamily: "var(--font-mono)" }}>
            AMOY
          </span>
        </div>
    ...
    <div style={{ display: "flex", height: "100vh", background: "var(--bg)", fontFamily: "var(--font-sans)", overflow: "hidden" }}>
      ...
      <motion.aside
        ...
        style={{
          ...
          boxShadow: "1px 0 0 var(--border)", position: "relative", zIndex: 10,
        }}>
        ...
        <div style={{
          ...
          gap: "8px", height: "60px", flexShrink: 0,
        }}>
          <AnimatePresence>
            {!collapsed && (
              <motion.div
                ...
                <div style={{
                  ...
                  background: "linear-gradient(135deg, #f5a623, #e8920f)",
                  display: "flex", alignItems: "center", justifyContent: "center",
                  flexShrink: 0, boxShadow: "0 3px 10px rgba(245,166,35,0.35)",
                }}>
                  <Ticket size={15} color="#fff" strokeWidth={2.5} />
                </div>
                <div>
                  <div style={{ fontWeight: 800, fontSize: "13px", color: "var(--text-primary)", letterSpacing: "-0.3px", whiteSpace: "nowrap" }}>
                    Master Events
                  </div>
                  <div style={{ display: "flex", alignItems: "center", gap: "4px", marginTop: "1px" }}>
                    <motion.div animate={{ scale: [1, 1.5, 1] }} transition={{ repeat: Infinity, duration: 2 }}
                      style={{ width: "4px", height: "4px", borderRadius: "50%", background: "#16a34a" }} />
                    <span style={{ fontSize: "8px", color: "#16a34a", fontWeight: 700, letterSpacing: "0.8px", fontFamily: "var(--font-mono)", whiteSpace: "nowrap" }}>
                      POLYGON AMOY
                    </span>
                  </div>
                </div>
        ...
        {/* ── User profile — ONLY here, not in topbar ── */}
            ...
            <div style={{ padding: "12px 14px" }}>
              <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                ...
                <div style={{ minWidth: 0, flex: 1 }}>
                  <div style={{ fontWeight: 700, fontSize: "13px", color: "var(--text-primary)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", lineHeight: 1.3 }}>
                    {currentUser?.first_name} {currentUser?.last_name}
                  </div>
                  <div style={{ fontSize: "10px", color: "var(--text-muted)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", lineHeight: 1.4 }}>
                    {currentUser?.email}
                  </div>
                </div>
              </div>
              <div style={{ marginTop: "8px" }}>
                <span style={{
                  ...
                  fontSize: "9px", fontWeight: 700, color: "#f5a623", letterSpacing: "0.5px",
                }}>
                  {role === "organizer" ? <><Zap size={9} color="#f5a623" /> ORGANIZER</> : <><Ticket size={9} color="#f5a623" /> ATTENDEE</>}
                </span>
              </div>
            </div>
          )}
        ...
        <nav style={{ flex: 1, padding: "8px", overflowY: "auto" }}>
          {!collapsed && (
            <div style={{ fontSize: "9px", fontWeight: 700, color: "var(--text-muted)", letterSpacing: "1.5px", padding: "8px 10px 6px", fontFamily: "var(--font-mono)" }}>
              NAVIGATE
            </div>
          )}
          {navItems.map(item => (
          ...
          {role === "attendee" && (
            <NavItem icon={Ticket} label="Resale Market"
              active={screen === "resaleMarket"} collapsed={collapsed}
              title={collapsed ? "Resale Market" : ""}
            ...
            <div style={{ marginTop: "8px" }}>
              ...
              <motion.div whileTap={{ scale: 0.94 }}
                ...
                style={{
                  ...
                  border: collapsed ? "none" : "1.5px dashed rgba(245,166,35,0.3)",
                  color: "#f5a623", transition: "all 0.18s ease", marginBottom: "3px",
                }}
                onMouseEnter={e => { e.currentTarget.style.background = "rgba(245,166,35,0.07)"; if (!collapsed) e.currentTarget.style.borderColor = "#f5a623"; }}
                ...
                <PlusCircle size={16} color="#f5a623" style={{ flexShrink: 0 }} />
      ...
      <main style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0, overflow: "hidden" }}>
        <DesktopTopbar
          ...
          setScreen={setScreen} setActiveTab={setActiveTab}
