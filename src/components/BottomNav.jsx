...
export default function BottomNav() {
  ...
  const attendeeTabs = [
    { id: "home",    Icon: Home,   label: "Discover", badge: 0 },
    { id: "tickets", Icon: Ticket, label: "Tickets",  badge: newTicketCount },
    { id: "alerts",  Icon: Bell,   label: "Alerts",   badge: 0 },
  ];
  const organizerTabs = [
    { id: "dashboard", Icon: LayoutDashboard, label: "Home",   badge: 0 },
    { id: "events",    Icon: CalendarDays,    label: "Events", badge: 0 },
        ...
        {showMenu && (
          <>
            ...
            <motion.div
              ...
              style={{ position: "fixed", bottom: 0, left: 0, right: 0, zIndex: 491, background: "var(--bg-card)", borderRadius: "24px 24px 0 0", boxShadow: "0 -12px 56px rgba(0,0,0,0.22)", border: "1px solid var(--border)", paddingBottom: "calc(env(safe-area-inset-bottom, 0px) + 8px)" }}>
      ...
      <nav style={{ position: "fixed", bottom: 0, left: 0, right: 0, zIndex: 300, background: "var(--bg-card)", backdropFilter: "blur(24px)", WebkitBackdropFilter: "blur(24px)", borderTop: "1px solid var(--border)", boxShadow: "0 -1px 0 var(--border), 0 -8px 24px rgba(0,0,0,0.05)", paddingBottom: "env(safe-area-inset-bottom, 0px)" }}>
        <div style={{ display: "flex", justifyContent: "space-around", alignItems: "center", height: `${NAV_HEIGHT}px` }}>
          {tabs.map(({ id, Icon, label, badge }) => {
            ...
            return (
              <motion.button key={id} whileTap={{ scale: 0.84 }}
                onClick={() => handleTabClick(id)}
                style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: "3px", flex: 1, height: "100%", cursor: "pointer", position: "relative", border: "none", background: "transparent", padding: 0, fontFamily: "var(--font-sans)" }}>
                ...
                <span style={{ fontSize: "10px", fontWeight: active ? 700 : 500, color: active ? "#f5a623" : "var(--text-muted)", letterSpacing: "0.1px", transition: "color 0.18s", position: "relative", zIndex: 1 }}>
                  {label}
                </span>
              </motion.button>
            );
