...
import {
  User, Envelope, Shield, SignOut, CaretRight, ArrowLeft,
  Bell, Globe, CheckCircle, PencilSimple, FloppyDisk, Link, Cookie,
  FileText, Lock, Eye, EyeSlash, Trash
...
function SectionHeader({ title }) {
  return (
    <div className="text-xs font-medium text-brand-muted uppercase tracking-widest pt-5 pb-2">
      {title}
    </div>
  );
}

function SettingRow({ icon: Icon, label, value, action, danger, onClick, toggle, checked, onToggle, badgeBg = "bg-blue-50", iconClass = "text-blue-700" }) {
  return (
    <div onClick={onClick}
      className={`flex items-center gap-3.5 px-4 py-3.5 bg-brand-card rounded-2xl border border-brand-hairline mb-1.5 transition-colors ${onClick ? "cursor-pointer hover:border-brand-hairline" : ""}`}>
      <div className={`w-9 h-9 rounded-full flex items-center justify-center shrink-0 ${danger ? "bg-red-50" : badgeBg}`}>
        <Icon size={16} weight="light" className={danger ? "text-red-600" : iconClass} />
      </div>
      ...
      {toggle ? (
        <button onClick={e => { e.stopPropagation(); onToggle(); }}
          className={`w-11 h-6 rounded-full relative shrink-0 transition-colors ${checked ? "bg-brand-accent" : "bg-gray-200"}`}>
          <motion.div animate={{ x: checked ? 20 : 2 }} transition={{ type: "spring", stiffness: 500, damping: 30 }}
            className="absolute top-0.5 w-5 h-5 rounded-full bg-white shadow" />
        </button>
      ) : onClick ? (
        <CaretRight size={16} weight="light" className="text-brand-muted shrink-0" />
      ) : action ? (
        <span className="text-xs font-semibold text-brand-accent shrink-0">{action}</span>
      ) : null}
    </div>
  );
}
  ...
  return (
    <>
      ...
      <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 40 }}
        ...
        className="fixed bottom-0 left-0 right-0 mx-auto z-[301] bg-brand-card rounded-t-2xl border border-brand-hairline p-6"
...
const fieldClass = "w-full px-3.5 py-2.5 rounded-xl border border-brand-hairline bg-brand-card text-sm text-brand-text outline-none focus:border-brand-accent focus:ring-2 focus:ring-brand-accent/20 transition-colors";
...
function AvatarPickerModal({ currentSeed, onSelect, onClose }) {
  ...
  return (
    <SheetShell onClose={onClose}>
      <div className="font-semibold text-lg text-brand-text mb-1.5">Choose Your Avatar</div>
          ...
          {AVATAR_PRESETS.map(seed => {
            ...
            return (
              <button key={seed} onClick={() => setSelected(seed)}
                className={`relative rounded-2xl p-1 border-2 transition-colors ${isSelected ? "border-brand-accent bg-[var(--brand-light)]" : "border-transparent"}`}>
                <Avatar seed={seed} size={44} style={{ width: "100%", height: "auto", aspectRatio: "1", borderRadius: "10px" }} />
                {isSelected && (
                  <span className="absolute top-0.5 right-0.5 w-4 h-4 rounded-full bg-brand-accent flex items-center justify-center border-2 border-white">
      ...
      <div className="flex items-center gap-3.5 bg-brand-canvas border border-brand-hairline rounded-2xl p-4 mb-3.5">
        <Avatar seed={selected} size={52} style={{ borderRadius: "50%", flexShrink: 0 }} />
        <div>
          <div className="text-sm font-medium text-brand-text">Preview</div>
          <div className="font-mono text-xs text-brand-muted mt-0.5">{selected}</div>
        </div>
      </div>
      <div className="flex gap-2.5">
        ...
        <button onClick={() => { onSelect(selected); onClose(); }}
          className="flex-[2] h-11 rounded-xl bg-brand-accent hover:bg-brand-accent-hover text-white font-medium text-sm transition-colors">
          Apply Avatar
        </button>
      </div>
    </SheetShell>
...
function PasswordModal({ onClose }) {
  ...
  return (
    <SheetShell onClose={onClose}>
      <div className="font-semibold text-lg text-brand-text mb-1.5">Change Password</div>
      ...
      <div className="mb-3">
        ...
        <div className="relative">
          ...
          <button onClick={() => setShowCur(!showCur)} type="button" className="absolute right-3.5 top-1/2 -translate-y-1/2 text-brand-muted">
            {showCur ? <EyeSlash size={15} weight="light" /> : <Eye size={15} weight="light" />}
          </button>
        </div>
      </div>
      ...
      <div className="mb-3">
        ...
        <div className="relative">
          ...
          <button onClick={() => setShowNew(!showNew)} type="button" className="absolute right-3.5 top-1/2 -translate-y-1/2 text-brand-muted">
            {showNew ? <EyeSlash size={15} weight="light" /> : <Eye size={15} weight="light" />}
          </button>
        </div>
      </div>
      ...
      <div className="flex gap-2.5">
        <button onClick={onClose} className="flex-1 h-11 rounded-xl bg-brand-canvas border border-brand-hairline font-medium text-sm text-brand-text">
          Cancel
        </button>
  ...
  return (
    <SheetShell onClose={onClose}>
      <div className="flex items-center gap-3 mb-3">
        ...
        <div>
          <div className="font-semibold text-lg text-red-600">Delete Account</div>
      ...
      <div className="flex gap-2.5">
        ...
        <button onClick={handleDelete} disabled={deleting || !password}
          className="flex-1 h-11 rounded-xl bg-red-50 border border-red-200 disabled:opacity-60 font-medium text-sm text-red-600">
  ...
  return (
    <div className="bg-brand-canvas min-h-full pb-14 font-sans">
      ...
      <div className={`sticky top-0 z-20 bg-brand-card border-b border-brand-hairline h-14 flex items-center justify-between ${desktop ? "px-10" : "px-4"}`}>
        ...
        <div className="font-semibold text-base text-brand-text tracking-tight">Settings</div>
        <div className="w-9" />
      </div>
      ...
      <div className={`mx-auto ${desktop ? "max-w-[640px] px-10 py-6" : "px-4 py-4"}`}>
        ...
        {/* ── Profile card ── */}
        <div className="bg-brand-card rounded-2xl border border-brand-hairline overflow-hidden mb-2">
          <div className="p-6 border-b border-brand-hairline">
            <div className="flex items-center gap-4">
              <button onClick={() => setShowPicker(true)} className="relative shrink-0">
                ...
                <span className="absolute bottom-0 right-0 w-[22px] h-[22px] rounded-full bg-brand-accent flex items-center justify-center border-2 border-white">
                  <PencilSimple size={10} className="text-white" />
                </span>
              </button>
              <div className="flex-1 min-w-0">
                <div className="font-semibold text-xl text-brand-text tracking-tight">
                  {displayFirst} {displayLast}
                </div>
                <div className="text-sm text-brand-muted mt-0.5">{currentUser?.email}</div>
                <span className="inline-flex items-center gap-1.5 mt-2 px-2.5 py-1 rounded-full bg-[var(--brand-light)]">
                  <span className="text-xs font-medium text-brand-accent">{(currentUser?.role || "ATTENDEE").toUpperCase()}</span>
                </span>
              </div>
            </div>

            <button onClick={() => setShowPicker(true)}
              className="w-full mt-3.5 flex items-center gap-2 px-3.5 py-2.5 bg-[var(--brand-light)] rounded-xl">
              <div className="flex gap-1">
                {AVATAR_PRESETS.slice(0, 5).map(seed => (
                  <Avatar key={seed} seed={seed} size={20}
                    style={{ borderRadius: "50%", border: seed === avatarSeed ? "2px solid #1c2e53" : "2px solid transparent" }} />
                ))}
                <div className="w-5 h-5 rounded-full bg-brand-card border border-brand-hairline flex items-center justify-center text-xs font-medium text-brand-muted">
                  +{AVATAR_PRESETS.length - 5}
                </div>
              </div>
              <span className="text-xs font-semibold text-brand-accent ml-1">Change Avatar</span>
              <CaretRight size={14} weight="light" className="text-brand-accent ml-auto" />
            </button>
          </div>
              ...
              <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} style={{ overflow: "hidden" }}>
                <div className="px-6 py-4 border-b border-brand-hairline">
                  ...
                  <div className="flex gap-2 mt-3.5">
                    <button onClick={handleSave} disabled={saving}
                      className="flex-[2] h-11 rounded-xl bg-brand-accent hover:bg-brand-accent-hover disabled:opacity-60 text-white font-medium text-sm flex items-center justify-center gap-1.5 transition-colors">
                      <FloppyDisk size={14} weight="light" /> {saving ? "Saving..." : "FloppyDisk Changes"}
                    </button>
                    <button onClick={() => { setEditing(false); setEditFirst(displayFirst); setEditLast(displayLast); }}
                      className="flex-1 h-11 rounded-xl bg-brand-canvas border border-brand-hairline text-brand-text font-medium text-sm">
                      Cancel
                    </button>
                  </div>
                </div>
          ...
          <div className="p-3">
            {!editing && (
              <button
                ...
                className="w-full h-11 bg-brand-canvas border border-brand-hairline rounded-xl text-brand-text font-medium text-sm flex items-center justify-center gap-1.5">
                <PencilSimple size={14} weight="light" /> Edit Profile
              </button>
            )}
          </div>
        </div>
        ...
        <SettingRow icon={Envelope} label="Email Address" value={currentUser?.email} badgeBg="bg-blue-50" iconClass="text-blue-700" />
        <SettingRow icon={Lock} label="Password" value="Change your account password" badgeBg="bg-blue-50" iconClass="text-blue-700" onClick={() => setShowPassword(true)} />

        <SectionHeader title="Notifications" />
        <SettingRow icon={Bell} label="Email Notifications" value="Ticket activity, NFT confirmations, sales" badgeBg="bg-[var(--brand-light)]" iconClass="text-brand-accent"
          toggle checked={notifs} onToggle={() => setNotifs(!notifs)} />

        <SectionHeader title="Blockchain" />
        <div className="bg-brand-card rounded-2xl border border-brand-hairline p-4 mb-1.5">
          <div className="flex items-center gap-3 mb-3">
            <div className="w-9 h-9 rounded-full bg-blue-50 flex items-center justify-center shrink-0">
              <Link size={16} weight="light" className="text-blue-700" />
            </div>
            <div>
              <div className="text-sm font-medium text-blue-700">Polygon Amoy Testnet</div>
              <div className="font-mono text-xs text-brand-muted mt-0.5">Chain ID: 80002 · NFT tickets auto-minted</div>
            </div>
          ...
          <div className="bg-brand-canvas border border-brand-hairline rounded-xl px-3 py-2.5 mb-2.5">
            <div className="text-xs text-brand-muted mb-1">NFT CONTRACT</div>
            <div className="text-xs font-medium text-blue-700 font-mono break-all">
        ...
        <SettingRow icon={FileText} label="Privacy Policy"     badgeBg="bg-brand-hairline" iconClass="text-brand-muted" onClick={() => setScreen("privacy")} />
        <SettingRow icon={Shield}   label="Terms of Service"   badgeBg="bg-brand-hairline" iconClass="text-brand-muted" onClick={() => setScreen("privacy")} />
        <SettingRow icon={Cookie}   label="Cookie Preferences" value="Manage what we track" badgeBg="bg-brand-hairline" iconClass="text-brand-muted" onClick={() => {
          localStorage.removeItem("me_cookie_consent");
          window.location.reload();
        }} />

        <SectionHeader title="About" />
        <SettingRow icon={Globe} label="Version" value="Master Events v1.0 · Built on Polygon" badgeBg="bg-blue-50" iconClass="text-blue-700" />

        <SectionHeader title="Account Actions" />
        <SettingRow icon={SignOut} label="Log Out" danger onClick={() => setShowLogout(true)} />
        <SettingRow icon={Trash} label="Delete Account" value="Permanently delete your account and all data" danger onClick={() => setShowDelete(true)} />

        <AnimatePresence>
          {showLogout && (
            <SheetShell onClose={() => setShowLogout(false)}>
              <div className="font-semibold text-lg text-brand-text mb-2">Log out?</div>
              ...
              <div className="flex gap-2.5">
                ...
                <button onClick={() => { setShowLogout(false); handleLogout(); }}
                  className="flex-1 h-11 rounded-xl bg-red-50 border border-red-200 font-medium text-sm text-red-600">
                  Log Out
                </button>
              </div>
            </SheetShell>
