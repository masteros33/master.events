      ...
      <div style={{ margin: "0 20px 20px", borderRadius: "20px", overflow: "hidden", boxShadow: "0 4px 20px rgba(0,0,0,0.4)" }}>
        <div style={{ height: "140px", position: "relative" }}>
          ...
          <div style={{ position: "absolute", bottom: "14px", left: "16px", right: "16px" }}>
            ...
            <div style={{ color: "rgba(255,255,255,0.7)", fontSize: "12px", marginTop: "4px" }}>📅 {checkoutEvent.date} · 📍 {checkoutEvent.venue}</div>
          </div>
        </div>
      </div>
      ...
      <div style={{ padding: "0 20px" }}>
        <div style={{ background: CARD, border: "1px solid " + BORDER, borderRadius: "20px", padding: "18px", marginBottom: "16px" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "20px" }}>
            <button onClick={() => setTicketQty(Math.max(1, ticketQty - 1))} style={{ width: "44px", height: "44px", borderRadius: "50%", background: "rgba(245,166,35,0.15)", color: "#f5a623", border: "2px solid rgba(245,166,35,0.3)", fontSize: "22px", fontWeight: 700, cursor: "pointer" }}>−</button>
            <span style={{ fontSize: "28px", fontWeight: 900, color: "#fff", minWidth: "40px", textAlign: "center" }}>{ticketQty}</span>
            <button onClick={() => setTicketQty(Math.min(10, ticketQty + 1))} style={{ width: "44px", height: "44px", borderRadius: "50%", background: "rgba(245,166,35,0.15)", color: "#f5a623", border: "2px solid rgba(245,166,35,0.3)", fontSize: "22px", fontWeight: 700, cursor: "pointer" }}>+</button>
          </div>
        </div>
        ...
        <div style={{ background: CARD, border: "1px solid " + BORDER, borderRadius: "20px", padding: "18px", marginBottom: "16px" }}>
          ...
          <div style={{ display: "flex", gap: "10px" }}>
            {[["momo", "📱 MoMo"], ["visa", "💳 VISA"]].map(([id, label]) => (
              <button key={id} onClick={() => setPayMethod(id)} style={{ flex: 1, padding: "14px", borderRadius: "14px", border: "2px solid " + (payMethod === id ? "#f5a623" : "rgba(255,255,255,0.1)"), background: payMethod === id ? "rgba(245,166,35,0.15)" : "rgba(255,255,255,0.04)", color: payMethod === id ? "#f5a623" : "rgba(255,255,255,0.4)", fontWeight: 700, fontSize: "14px", cursor: "pointer" }}>{label}</button>
            ))}
          </div>
        </div>

        <div style={{ background: CARD, border: "1px solid " + BORDER, borderRadius: "20px", padding: "18px", marginBottom: "20px" }}>
          <div style={{ fontWeight: 700, fontSize: "14px", color: "#fff", marginBottom: "14px" }}>Order Summary</div>
          {[
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            ...
            <span style={{ color: "#f5a623", fontWeight: 900, fontSize: "22px" }}>Ghc {total}</span>
          </div>
        </div>

        {paying && (
          <div style={{ background: "rgba(245,166,35,0.08)", border: "1px solid rgba(245,166,35,0.25)", borderRadius: "20px", padding: "24px 20px", marginBottom: "16px", textAlign: "center" }}>
            <div style={{ fontSize: "40px", marginBottom: "10px" }}>⏳</div>
            <div style={{ color: "#f5a623", fontWeight: 700, fontSize: "16px", marginBottom: "6px" }}>Processing Payment...</div>
            <div style={{ color: "rgba(255,255,255,0.4)", fontSize: "12px" }}>Minting your NFT ticket on Polygon</div>
          </div>
        )}
  ...
  return (
    <div style={{ background: BG, minHeight: "100%", paddingBottom: "100px" }}>
      <div style={{ height: "220px", position: "relative" }}>
        ...
        <div style={{ position: "absolute", inset: 0, background: "linear-gradient(to bottom, rgba(17,9,0,0.3) 0%, rgba(17,9,0,0.95) 100%)" }} />
      ...
      <div style={{ background: "rgba(26,14,0,0.95)", margin: "16px", borderRadius: "24px", overflow: "hidden", boxShadow: "0 8px 40px rgba(0,0,0,0.6)", border: "1px solid rgba(245,166,35,0.25)" }}>
        {/* Ticket info */}
        <div style={{ padding: "20px", display: "flex", justifyContent: "space-around", borderBottom: "1px dashed rgba(245,166,35,0.2)" }}>
          {[
            ...
            <div key={k} style={{ textAlign: "center" }}>
              <div style={{ fontSize: "10px", color: "rgba(255,255,255,0.35)", fontWeight: 600, marginBottom: "6px", letterSpacing: "1px" }}>{k}</div>
              <div style={{ fontWeight: 800, fontSize: "15px", color: "#fff" }}>{v}</div>
            </div>
          ))}
        </div>
        ...
        <div style={{ padding: "24px 20px", display: "flex", flexDirection: "column", alignItems: "center", gap: "14px" }}>
          {qrSrc ? (
            <div style={{ position: "relative" }}>
              {!qrLoaded && !qrError && (
                <div style={{ width: "200px", height: "200px", borderRadius: "16px", background: "rgba(255,255,255,0.05)", display: "flex", alignItems: "center", justifyContent: "center", position: "absolute", top: 0, left: 0 }}>
                  <div style={{ color: "rgba(255,255,255,0.3)", fontSize: "13px" }}>Loading QR...</div>
                </div>
              )}
              <img
                src={qrSrc}
                ...
                style={{
                  ...
                  borderRadius: "16px",
                  border: "4px solid rgba(245,166,35,0.6)",
                  background: "#fff",
                  padding: "6px",
              ...
              />
              {qrError && (
                <div style={{ width: "200px", height: "200px", borderRadius: "16px", background: "rgba(255,255,255,0.05)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", border: "1px solid rgba(245,166,35,0.2)", gap: "8px" }}>
                  <span style={{ fontSize: "40px" }}>📱</span>
                  <span style={{ fontSize: "11px", color: "rgba(255,255,255,0.4)" }}>QR unavailable</span>
                </div>
              )}
            </div>
          ) : (
            <div style={{ width: "200px", height: "200px", borderRadius: "16px", background: "rgba(255,255,255,0.05)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", border: "1px solid rgba(245,166,35,0.2)", gap: "10px" }}>
              <span style={{ fontSize: "48px" }}>📱</span>
              <span style={{ fontSize: "12px", color: "rgba(255,255,255,0.4)" }}>No QR available</span>
            </div>
          )}

          <div style={{ fontFamily: "monospace", fontSize: "13px", color: "rgba(255,255,255,0.4)", letterSpacing: "2px", background: "rgba(255,255,255,0.05)", padding: "8px 16px", borderRadius: "20px" }}>
            {viewingTicket.id}
          </div>

          <div style={{ background: "rgba(39,174,96,0.12)", border: "1px solid rgba(39,174,96,0.3)", padding: "8px 20px", borderRadius: "20px", display: "flex", alignItems: "center", gap: "6px" }}>
        ...
        <div style={{ padding: "0 16px 16px", display: "flex", gap: "10px" }}>
          <button onClick={() => { setResaleTicket(viewingTicket); setResalePrice(""); setResaleError(""); setScreen("resale"); }}
            style={{ flex: 1, padding: "14px", background: "rgba(245,166,35,0.1)", color: "#f5a623", border: "2px solid rgba(245,166,35,0.35)", borderRadius: "50px", fontSize: "13px", fontWeight: 700, cursor: "pointer" }}>🏷️ RESELL</button>
          <button onClick={() => { setTransferTicket(viewingTicket); setTransferEmail(""); setTransferName(""); setTransferDone(false); setScreen("transfer"); }}
            style={{ flex: 1, padding: "14px", background: "rgba(255,255,255,0.05)", color: "rgba(255,255,255,0.8)", border: "1px solid rgba(255,255,255,0.12)", borderRadius: "50px", fontSize: "13px", fontWeight: 700, cursor: "pointer" }}>📤 TRANSFER</button>
        </div>
        <div style={{ padding: "0 16px 20px" }}>
          <button onClick={() => { setScreen("app"); setActiveTab("home"); }}
            style={{ width: "100%", padding: "14px", background: "rgba(39,174,96,0.15)", border: "1px solid rgba(39,174,96,0.35)", color: "#27ae60", borderRadius: "50px", fontWeight: 700, cursor: "pointer", fontSize: "14px" }}>✅ DONE</button>
        </div>
      </div>
    </div>
  );
    ...
    <div style={{ background: BG, minHeight: "100%", paddingBottom: "100px" }}>
      ...
      <div style={{ padding: "0 20px" }}>
        <div style={{ background: CARD, border: "1px solid " + BORDER, borderRadius: "20px", padding: "16px", marginBottom: "16px" }}>
          <div style={{ fontWeight: 700, fontSize: "15px", color: "#fff" }}>{ev.name}</div>
        ...
        <div style={{ background: CARD, border: "1px solid " + BORDER, borderRadius: "20px", padding: "18px", marginBottom: "16px" }}>
          ...
          <input value={resalePrice} onChange={e => setResalePrice(e.target.value)} type="number"
            ...
            style={{ ...darkInput, fontSize: "22px", fontWeight: 800, border: "2px solid " + (resaleError ? "#e74c3c" : "rgba(245,166,35,0.4)") }} />
          {resaleError && <div style={{ color: "#ff6b6b", fontSize: "12px", marginTop: "-4px", marginBottom: "8px" }}>⚠️ {resaleError}</div>}
        </div>
        ...
        {price > 0 && (
          <div style={{ background: CARD, border: "1px solid " + BORDER, borderRadius: "20px", padding: "18px", marginBottom: "16px" }}>
            ...
            {[
              ["Listing Price", `Ghc ${price}`, "#fff"],
              ["Platform Fee (2%)", `− Ghc ${fee}`, "#ff6b6b"],
              ["Your Payout", `Ghc ${payout}`, "#27ae60"]
            ].map(([k, v, c], i) => (
              <div key={k} style={{ display: "flex", justifyContent: "space-between", padding: "10px 0", borderBottom: i < 2 ? "1px solid rgba(255,255,255,0.06)" : "none" }}>
                <span style={{ color: "rgba(255,255,255,0.5)", fontSize: "13px" }}>{k}</span>
        ...
        <button onClick={handleListForResale} style={darkBtn}>🏷️ LIST FOR RESALE</button>
      </div>
    </div>
  );
}
...
export function ResaleSuccess() {
  ...
  return (
    <div style={{ background: BG, minHeight: "100%", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "40px 28px", textAlign: "center" }}>
      ...
      <div style={{ fontSize: "26px", fontWeight: 900, color: "#fff", marginBottom: "10px" }}>Listed for Resale!</div>
      <div style={{ color: "rgba(255,255,255,0.5)", fontSize: "14px", lineHeight: 1.8, marginBottom: "28px" }}>Your ticket is now on the resale market. You'll be notified when it sells.</div>
      <div style={{ background: "rgba(39,174,96,0.08)", border: "1px solid rgba(39,174,96,0.25)", borderRadius: "20px", padding: "20px", marginBottom: "28px", width: "100%", textAlign: "left" }}>
...
export function Transfer() {
  ...
  if (transferDone) return (
    <div style={{ background: BG, minHeight: "100%", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "40px 28px", textAlign: "center" }}>
      ...
      <div style={{ fontSize: "26px", fontWeight: 900, color: "#fff", marginBottom: "10px" }}>Ticket Transferred!</div>
      <div style={{ color: "rgba(255,255,255,0.5)", fontSize: "14px", lineHeight: 1.8, marginBottom: "20px" }}>
      ...
      <div style={{ background: "rgba(39,174,96,0.08)", border: "1px solid rgba(39,174,96,0.25)", borderRadius: "20px", padding: "20px", marginBottom: "28px", width: "100%", textAlign: "left" }}>
        ...
        <div style={{ fontSize: "12px", color: "rgba(255,255,255,0.4)" }}>Your old QR code is now void. Recipient has a new NFT ticket.</div>
      </div>
  ...
  return (
    <div style={{ background: BG, minHeight: "100%", paddingBottom: "100px" }}>
      ...
      <div style={{ padding: "0 20px" }}>
        <div style={{ background: "rgba(41,128,185,0.08)", border: "1px solid rgba(41,128,185,0.25)", borderRadius: "16px", padding: "16px 18px", marginBottom: "20px" }}>
          <div style={{ fontWeight: 700, fontSize: "13px", color: "#5dade2", marginBottom: "10px" }}>ℹ️ About Transfers</div>
          {[
        ...
        <div style={{ fontSize: "13px", color: "rgba(255,255,255,0.5)", fontWeight: 600, marginBottom: "8px" }}>Recipient Email</div>
        <input placeholder="e.g. kwame@email.com" value={transferEmail} onChange={e => setTransferEmail(e.target.value)} style={darkInput} />

        <div style={{ background: "rgba(231,76,60,0.08)", border: "1px solid rgba(231,76,60,0.2)", borderRadius: "12px", padding: "12px 16px", marginBottom: "24px" }}>
          <div style={{ fontSize: "12px", color: "#ff6b6b", fontWeight: 700, marginBottom: "4px" }}>⚠️ This cannot be undone</div>
          <div style={{ fontSize: "12px", color: "rgba(255,255,255,0.4)" }}>Make sure the email address is correct before confirming.</div>
        </div>
