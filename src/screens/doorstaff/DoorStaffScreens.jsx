  ...
  return (
    <motion.div
      ...
      <motion.div
        ...
        style={{ fontSize: "56px", marginBottom: "12px", lineHeight: 1 }}>
        {result.icon}
      </motion.div>
      ...
      {result.holder && (
        <div style={{ color: "#d0cdc8", fontSize: "15px", fontWeight: 600, marginBottom: "5px" }}>
          👤 {result.holder}
        </div>
      )}
      {result.event_name && (
      ...
      <motion.div
        ...
        <div style={{ textAlign: "center", marginBottom: "32px" }}>
          <motion.div
            ...
            style={{ width: "80px", height: "80px", borderRadius: "24px", background: `linear-gradient(135deg, ${BRAND}, ${BRAND_DARK})`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: "36px", margin: "0 auto 20px", boxShadow: `0 12px 36px ${BRAND}40` }}>
            🚪
          </motion.div>
        ...
        <div style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: "24px", padding: "28px", backdropFilter: "blur(16px)" }}>
          ...
          <input
            ...
            style={{
              width: "100%", padding: "18px 20px", outline: "none",
              marginBottom: "12px",
          ...
          <AnimatePresence>
            {doorCodeError && (
              <motion.div
                ...
                style={{ color: "#f87171", fontSize: "13px", fontWeight: 600, marginBottom: "14px", background: "rgba(248,113,113,0.08)", border: "1px solid rgba(248,113,113,0.2)", borderRadius: "10px", padding: "11px 14px" }}>
                ⚠️ {doorCodeError}
              </motion.div>
            )}
          </AnimatePresence>
          ...
          <motion.button
            ...
            style={{ width: "100%", padding: "16px", borderRadius: "14px", background: `linear-gradient(135deg, ${BRAND}, ${BRAND_DARK})`, color: "#fff", fontWeight: 800, fontSize: "16px", border: "none", cursor: "pointer", boxShadow: `0 8px 24px ${BRAND}40`, fontFamily: "var(--font-sans)", letterSpacing: "-0.2px" }}>
            ENTER EVENT →
          </motion.button>
        </div>
...
export function DoorStaffScan() {
  ...
  const processId = async (id) => {
    ...
    try {
      ...
      setResult({ status: "error", color: "#dc2626", icon: "❌", title: "Verification Failed", msg: "Could not verify. Check your connection.", holder: null });
    }
      ...
      <div style={{ background: "#1a1916", padding: "14px 18px", display: "flex", justifyContent: "space-between", alignItems: "center", borderBottom: "1px solid #2a2826", position: "sticky", top: 0, zIndex: 10, boxShadow: "0 4px 16px rgba(0,0,0,0.3)" }}>
        <div>
          <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
            <div style={{ width: "34px", height: "34px", borderRadius: "10px", background: `linear-gradient(135deg, ${BRAND}, ${BRAND_DARK})`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: "16px", boxShadow: `0 4px 12px ${BRAND}40` }}>🔍</div>
            <div>
        ...
        <motion.button whileTap={{ scale: 0.95 }} onClick={() => { setScreen("home"); handleLogout(); }}
          style={{ padding: "8px 16px", background: "rgba(220,38,38,0.1)", color: "#f87171", border: "1px solid rgba(220,38,38,0.25)", borderRadius: "99px", fontSize: "12px", fontWeight: 700, cursor: "pointer", fontFamily: "var(--font-sans)" }}>
          Exit
        </motion.button>
      </div>
        ...
        {cameraMode ? (
          ...
          <div style={{ background: "#1a1916", borderRadius: "18px", padding: "16px", marginBottom: "16px", border: "1px solid #2a2826" }}>
            ...
            <motion.button whileTap={{ scale: 0.97 }} onClick={() => processId(scanInput)}
              style={{ width: "100%", padding: "14px", background: `linear-gradient(135deg, ${BRAND}, ${BRAND_DARK})`, color: "#fff", border: "none", borderRadius: "12px", fontWeight: 700, fontSize: "14px", cursor: "pointer", opacity: verifying ? 0.7 : 1, fontFamily: "var(--font-sans)", boxShadow: `0 6px 18px ${BRAND}35` }}>
        ...
        {admittedList.length > 0 && (
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "12px" }}>
              <div style={{ color: "#fff", fontWeight: 700, fontSize: "14px" }}>✅ Admitted This Session</div>
    ...
    <div style={{ background: "var(--bg)", minHeight: "100%", paddingBottom: "40px", fontFamily: "var(--font-sans)" }}>
      <div style={{ display: "flex", alignItems: "center", padding: "16px 20px", gap: "14px", background: "var(--bg-card)", borderBottom: "1px solid var(--border)", position: "sticky", top: 0, zIndex: 10, boxShadow: "var(--shadow-sm)" }}>
        <motion.button whileTap={{ scale: 0.9 }} onClick={() => setScreen("orgEventDetail")}
          style={{ width: "38px", height: "38px", borderRadius: "11px", background: "var(--bg-subtle)", border: "1px solid var(--border)", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", fontSize: "17px", color: "var(--text-primary)", flexShrink: 0 }}>
          ←
        </motion.button>
        <div>
          ...
          <div style={{ fontSize: "12px", color: "var(--text-muted)", marginTop: "2px", display: "flex", alignItems: "center", gap: "5px" }}>
            <span style={{ color: "#a78bfa" }}>⛓️</span> Real-time blockchain verification
          </div>
        </div>
      </div>
      ...
      <div style={{ padding: "16px" }}>
        ...
        <div style={{ display: "flex", gap: "8px", marginBottom: "16px", background: "var(--bg-subtle)", borderRadius: "14px", padding: "4px" }}>
          {[["📷 Camera", true], ["⌨️ Manual", false]].map(([label, mode]) => (
            <motion.button key={label} whileTap={{ scale: 0.96 }} onClick={() => setCameraMode(mode)}
              style={{ flex: 1, padding: "10px", borderRadius: "10px", fontWeight: 700, fontSize: "13px", cursor: "pointer", border: "none", background: cameraMode === mode ? `linear-gradient(135deg, ${BRAND}, ${BRAND_DARK})` : "transparent", color: cameraMode === mode ? "#fff" : "var(--text-muted)", fontFamily: "var(--font-sans)", transition: "all 0.2s", boxShadow: cameraMode === mode ? `0 4px 12px ${BRAND}35` : "none" }}>
              {label}
            </motion.button>
          ))}
        </div>
        ...
        {cameraMode ? (
          ...
          <div style={{ background: "var(--bg-card)", borderRadius: "16px", padding: "16px", marginBottom: "16px", border: "1px solid var(--border)", boxShadow: "var(--shadow-sm)" }}>
            ...
            />
            <motion.button whileTap={{ scale: 0.97 }} onClick={() => processId(scanInput)}
              style={{ width: "100%", padding: "14px", background: `linear-gradient(135deg, ${BRAND}, ${BRAND_DARK})`, color: "#fff", border: "none", borderRadius: "12px", fontWeight: 700, fontSize: "14px", cursor: "pointer", opacity: verifying ? 0.7 : 1, fontFamily: "var(--font-sans)", boxShadow: `0 6px 18px ${BRAND}30` }}>
              {verifying ? "⛓️ Verifying..." : "🔍 VERIFY TICKET"}
            </motion.button>
          </div>
        )}
        ...
        {result && (
          <motion.div
            ...
            {result.holder && <div style={{ color: "var(--text-primary)", fontSize: "15px", fontWeight: 600, marginBottom: "4px" }}>👤 {result.holder}</div>}
            {result.event_name && <div style={{ color: "var(--text-muted)", fontSize: "13px", marginBottom: "8px" }}>🎟️ {result.event_name}</div>}
            {result.status === "valid" && (
...
function buildResult(data, rawId) {
  if (data.error || !data.valid) {
    if (data.status === "redeemed" || data.reason === "Already redeemed") {
      return { status: "redeemed", color: "#dc2626", icon: "🚫", title: "Already Redeemed", msg: "Do not admit. This ticket was already scanned.", holder: data.holder || null, event_name: data.event_name || null };
    }
    if (data.status === "wrong_event") {
      return { status: "wrong_event", color: "#d97706", icon: "⚠️", title: "Wrong Event", msg: "This ticket is for: " + (data.event_name || "a different event"), holder: data.holder || null, event_name: data.event_name || null };
    }
    return { status: "invalid", color: "#dc2626", icon: "❌", title: "Invalid Ticket", msg: data.error || data.reason || "Could not verify this ticket.", holder: data.holder || null, event_name: data.event_name || null };
  }
  return {
