// ── OrganizerScreens.jsx ──────────────────────────────────────
import React, { useEffect, useState } from "react";
import { motion, AnimatePresence } from "framer-motion";
import useStore from "../../store/useStore";
import { eventsAPI } from "../../api";

// ── Google Sans Flex font injection ──────────────────────────
const FONT_LINK = document.getElementById("google-sans-flex");
if (!FONT_LINK) {
  const link = document.createElement("link");
  link.id   = "google-sans-flex";
  link.rel  = "stylesheet";
  link.href = "https://fonts.googleapis.com/css2?family=Google+Sans+Flex:wght@100..900&family=Google+Sans:wght@400;500;700&family=Google+Sans+Display:wght@400;500;700&display=swap";
...
// ── Google Sans Flex used for headings + display text ─────────
const FONT_DISPLAY = "'Google Sans Flex', 'Google Sans Display', 'Google Sans', var(--font-sans)";
const FONT_BODY    = "'Google Sans', var(--font-sans)";
...
const inp = {
  ...
  caretColor: "#f5a623", transition: "all 0.2s",
};
...
function OrgTelemetry({ events, totalSold, totalRevenue }) {
  ...
  return (
    <div style={{
      ...
      fontFamily: FONT_MONO, flexWrap: "wrap", marginBottom: "20px",
    }}>
      {[
...
function BentoStat({ icon, value, label, sub, color, bg, large, onClick }) {
  return (
    <motion.div
      whileHover={{ y: -3, boxShadow: "var(--shadow-lg)", borderColor: color + "40" }}
      whileTap={onClick ? { scale: 0.98 } : {}}
      onClick={onClick}
      style={{
        background: "var(--bg-card)", border: "1px solid var(--border)",
        borderRadius: "18px", padding: large ? "24px" : "18px",
        boxShadow: "var(--shadow-sm)", transition: "all 0.22s var(--ease-smooth)",
      ...
      <div style={{ fontSize: large ? "28px" : "20px", fontWeight: 900, color, letterSpacing: "-0.5px", marginBottom: "3px", lineHeight: 1, fontFamily: FONT_DISPLAY }}>
        {value}
      </div>
      <div style={{ fontSize: "11px", color: "var(--text-muted)", fontFamily: FONT_MONO }}>{label}</div>
      {sub && (
        <div style={{ marginTop: "6px", fontSize: "10px", fontWeight: 700, color, background: color + "12", padding: "2px 8px", borderRadius: "99px", width: "fit-content", fontFamily: FONT_MONO }}>
          {sub}
        </div>
      )}
    </motion.div>
...
function EventCard({ ev, onClick }) {
  ...
  return (
    <motion.div
      whileHover={{ y: -4, boxShadow: "var(--shadow-lg)", borderColor: "rgba(245,166,35,0.4)" }}
      ...
      <div style={{ height: "160px", position: "relative" }}>
        ...
        <div style={{ position: "absolute", top: "10px", right: "10px", background: ev.salesOpen ? "#16a34a" : "#6b6b6b", color: "#fff", fontSize: "9px", fontWeight: 700, padding: "3px 9px", borderRadius: "99px", display: "flex", alignItems: "center", gap: "4px", fontFamily: FONT_MONO }}>
        ...
        <div style={{ position: "absolute", top: "10px", left: "10px", background: "rgba(124,58,237,0.85)", backdropFilter: "blur(6px)", color: "#fff", fontSize: "8px", fontWeight: 700, padding: "3px 8px", borderRadius: "99px", display: "flex", alignItems: "center", gap: "3px", fontFamily: FONT_MONO }}>
          <span>⛓️</span><span>NFT</span>
        </div>
        <div style={{ position: "absolute", bottom: "10px", left: "12px", right: "12px" }}>
          <div style={{ color: "#fff", fontWeight: 800, fontSize: "14px", marginBottom: "2px", lineHeight: 1.2, fontFamily: FONT_DISPLAY }}>{ev.name}</div>
          <div style={{ color: "rgba(255,255,255,0.65)", fontSize: "10px", fontFamily: FONT_MONO }}>📍 {ev.venue} · {ev.date}</div>
        </div>
      </div>
      <div style={{ padding: "12px 14px" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "8px" }}>
          <span style={{ fontSize: "12px", color: "#16a34a", fontWeight: 700, fontFamily: FONT_MONO }}>GHS {Math.round(ev.ticketsSold * ev.price * 0.95).toLocaleString()}</span>
          <span style={{ fontSize: "10px", color: "var(--text-muted)", fontFamily: FONT_MONO }}>{ev.ticketsSold}/{ev.totalTickets}</span>
        </div>
  ...
  return (
    <div style={{ background: "var(--bg)", minHeight: "100%", padding: desktop ? "28px 40px 60px" : "16px 16px 120px" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "20px" }}>
        <div>
          <div style={{ fontSize: "9px", color: "#f5a623", fontWeight: 700, letterSpacing: "2px", marginBottom: "5px", fontFamily: FONT_MONO }}>ORGANIZER_DASHBOARD</div>
          <h1 style={{ fontWeight: 900, fontSize: desktop ? "28px" : "22px", color: "var(--text-primary)", letterSpacing: "-0.8px", marginBottom: "3px", fontFamily: FONT_DISPLAY }}>
            {desktop ? `Welcome back, ${currentUser?.first_name}` : "Dashboard"}
          </h1>
          <p style={{ color: "var(--text-muted)", fontSize: "13px", fontFamily: FONT_BODY }}>
        ...
        <div style={{ display: "flex", gap: "8px", flexShrink: 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: "5px", padding: "5px 11px", borderRadius: "99px", background: "rgba(124,58,237,0.06)", border: "1px solid rgba(124,58,237,0.15)" }}>
            ...
            <span style={{ fontSize: "9px", fontWeight: 700, color: "#7c3aed", fontFamily: FONT_MONO }}>POLYGON</span>
          </div>
          {desktop && (
            <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}
              onClick={() => setScreen("addEvent")}
              style={{ padding: "8px 18px", background: "linear-gradient(135deg, #f5a623, #e8920f)", color: "#fff", border: "none", borderRadius: "10px", fontSize: "12px", fontWeight: 700, cursor: "pointer", boxShadow: "var(--shadow-brand)", fontFamily: FONT_BODY }}>
              + New Event
            </motion.button>
          )}
        </div>
      ...
      {loading ? (
        ...
        <>
          ...
          {desktop ? (
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "14px", marginBottom: "24px" }}>
              ...
              <div style={{ gridColumn: "span 2" }}>
                <motion.div whileHover={{ y: -3, boxShadow: "var(--shadow-lg)" }}
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "14px" }}>
                    ...
                    <div>
                      <div style={{ fontSize: "13px", fontWeight: 700, color: "#7c3aed", fontFamily: FONT_DISPLAY }}>Polygon Amoy</div>
                      <div style={{ fontSize: "9px", color: "var(--text-muted)", fontFamily: FONT_MONO }}>CONTRACT: 0x956F...0Daf</div>
                    </div>
                    <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: "4px" }}>
                      <motion.div animate={{ opacity: [0.4,1,0.4] }} transition={{ duration: 2, repeat: Infinity }}
                        style={{ width: "6px", height: "6px", borderRadius: "50%", background: "#4ade80" }} />
                      <span style={{ fontSize: "9px", fontWeight: 700, color: "#4ade80", fontFamily: FONT_MONO }}>ONLINE</span>
                    </div>
                  </div>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "10px" }}>
                    {[["NFT_MINTED", totalSold.toString(), "#a78bfa"], ["CHAIN_ID", "80002", "#60a5fa"], ["GAS_COST", "~0.0002", "#4ade80"]].map(([key, val, color]) => (
                      <div key={key} style={{ background: "var(--bg-subtle)", borderRadius: "10px", padding: "10px 12px", border: "1px solid var(--border)" }}>
                        <div style={{ fontSize: "14px", fontWeight: 800, color, marginBottom: "2px", fontFamily: FONT_DISPLAY }}>{val}</div>
                        <div style={{ fontSize: "8px", color: "var(--text-muted)", fontFamily: FONT_MONO }}>{key}</div>
                      </div>
                    ))}
                  </div>
                </motion.div>
              ...
              <motion.div whileHover={{ y: -3, boxShadow: "var(--shadow-lg)" }}
                ...
                <div style={{ fontSize: "9px", fontWeight: 700, color: "var(--text-muted)", letterSpacing: "1.5px", marginBottom: "12px", fontFamily: FONT_MONO }}>PAYOUT_SPLIT</div>
                <div style={{ marginBottom: "10px" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "5px" }}>
                    <span style={{ fontSize: "11px", fontWeight: 700, color: "#16a34a", fontFamily: FONT_BODY }}>You (95%)</span>
                    <span style={{ fontSize: "11px", fontWeight: 700, color: "#16a34a", fontFamily: FONT_MONO }}>GHS {Math.round(totalRevenue).toLocaleString()}</span>
                  </div>
                ...
                <div>
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "5px" }}>
                    <span style={{ fontSize: "11px", color: "var(--text-muted)", fontFamily: FONT_BODY }}>Platform (5%)</span>
                    <span style={{ fontSize: "11px", color: "var(--text-muted)", fontFamily: FONT_MONO }}>GHS {Math.round(totalRevenue * 0.053).toLocaleString()}</span>
                  </div>
                  <div style={{ height: "6px", background: "var(--bg-subtle)", borderRadius: "3px", overflow: "hidden" }}>
              ...
              {topEvent && (
                <motion.div whileHover={{ y: -3, boxShadow: "var(--shadow-lg)", borderColor: "rgba(245,166,35,0.3)" }} whileTap={{ scale: 0.98 }}
                  ...
                  <div style={{ position: "relative", zIndex: 1 }}>
                    <div style={{ fontSize: "9px", fontWeight: 700, color: "#f5a623", letterSpacing: "1.5px", marginBottom: "8px", fontFamily: FONT_MONO }}>TOP_EVENT</div>
                    <div style={{ fontWeight: 800, fontSize: "13px", color: "var(--text-primary)", marginBottom: "4px", lineHeight: 1.3, fontFamily: FONT_DISPLAY }}>{topEvent.name}</div>
                    <div style={{ fontSize: "10px", color: "var(--text-muted)", fontFamily: FONT_MONO, marginBottom: "10px" }}>{topEvent.ticketsSold} sold · GHS {Math.round(topEvent.ticketsSold * topEvent.price * 0.95).toLocaleString()}</div>
                    <div style={{ height: "3px", background: "var(--bg-subtle)", borderRadius: "2px", overflow: "hidden" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: desktop ? "1fr 1fr" : "1fr", gap: "12px", marginBottom: "24px" }}>
            {[
              { icon: "💡", color: "#f5a623", bg: "rgba(245,166,35,0.04)", border: "rgba(245,166,35,0.15)", title: "Payouts via Paystack", body: "95% of ticket revenue goes straight to your wallet. Withdraw to MoMo anytime." },
              { icon: "⛓️", color: "#7c3aed", bg: "rgba(124,58,237,0.04)", border: "rgba(124,58,237,0.12)", title: "NFT Tickets on Polygon Amoy", body: "Every ticket minted on-chain. Zero fakes. QR rotates every 10s.", gradient: true },
            ].map((b, i) => (
              <div key={i} style={{ background: b.bg, border: "1px solid " + b.border, borderRadius: "14px", padding: "14px 16px", display: "flex", alignItems: "center", gap: "12px" }}>
                <div style={{ width: "34px", height: "34px", borderRadius: "10px", background: b.gradient ? "linear-gradient(135deg, #7c3aed, #2563eb)" : "rgba(245,166,35,0.1)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "15px", flexShrink: 0 }}>{b.icon}</div>
                <div>
                  <div style={{ fontSize: "12px", fontWeight: 700, color: b.color, marginBottom: "2px", fontFamily: FONT_DISPLAY }}>{b.title}</div>
                  <div style={{ fontSize: "11px", color: "var(--text-secondary)", lineHeight: 1.5, fontFamily: FONT_BODY }}>{b.body}</div>
                </div>
              </div>
            ))}
          </div>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "14px" }}>
            <div style={{ fontWeight: 800, fontSize: "17px", color: "var(--text-primary)", letterSpacing: "-0.4px", fontFamily: FONT_DISPLAY }}>
              Your Events
              {orgEvents.length > 0 && <span style={{ marginLeft: "8px", fontSize: "11px", color: "var(--text-muted)", fontWeight: 500, fontFamily: FONT_MONO }}>[{orgEvents.length}]</span>}
            </div>
            {!desktop && (
              <motion.button whileTap={{ scale: 0.95 }} onClick={() => setScreen("addEvent")}
                style={{ padding: "8px 16px", background: "linear-gradient(135deg, #f5a623, #e8920f)", color: "#fff", border: "none", borderRadius: "10px", fontSize: "12px", fontWeight: 700, cursor: "pointer", fontFamily: FONT_BODY }}>
                + New
              </motion.button>
            )}
          </div>
          ...
          {orgEvents.length === 0 ? (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
              ...
              <div style={{ fontWeight: 700, fontSize: "18px", color: "var(--text-primary)", marginBottom: "8px", fontFamily: FONT_DISPLAY }}>No events yet</div>
              <div style={{ fontSize: "13px", color: "var(--text-muted)", marginBottom: "20px", fontFamily: FONT_BODY }}>Create your first event and start selling NFT tickets</div>
              <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }} onClick={() => setScreen("addEvent")}
                style={{ padding: "12px 28px", background: "linear-gradient(135deg, #f5a623, #e8920f)", color: "#fff", border: "none", borderRadius: "12px", fontSize: "14px", fontWeight: 700, cursor: "pointer", boxShadow: "var(--shadow-brand)", fontFamily: FONT_BODY }}>
  ...
  return (
    <div style={{ background: "var(--bg)", minHeight: "100%", padding: desktop ? "28px 40px 60px" : "16px 16px 120px" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px" }}>
        <div>
          <div style={{ fontSize: "9px", color: "#f5a623", fontWeight: 700, letterSpacing: "2px", marginBottom: "4px", fontFamily: FONT_MONO }}>MY_EVENTS</div>
          <h1 style={{ fontWeight: 900, fontSize: desktop ? "26px" : "22px", color: "var(--text-primary)", letterSpacing: "-0.6px", fontFamily: FONT_DISPLAY }}>Events</h1>
        </div>
        <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }} onClick={() => setScreen("addEvent")}
          style={{ padding: "9px 18px", background: "linear-gradient(135deg, #f5a623, #e8920f)", color: "#fff", border: "none", borderRadius: "10px", fontSize: "12px", fontWeight: 700, cursor: "pointer", boxShadow: "var(--shadow-brand)", fontFamily: FONT_BODY }}>
          + New Event
        </motion.button>
      </div>
      ...
      {loading ? (
        ...
        <div style={{ textAlign: "center", padding: "60px 40px", background: "var(--bg-card)", borderRadius: "20px", border: "1px solid var(--border)" }}>
          <div style={{ fontSize: "48px", marginBottom: "12px" }}>🎪</div>
          <div style={{ fontWeight: 700, fontSize: "16px", color: "var(--text-primary)", marginBottom: "8px", fontFamily: FONT_DISPLAY }}>No events yet</div>
          <div style={{ fontSize: "13px", color: "var(--text-muted)", fontFamily: FONT_BODY }}>Click + New Event to get started</div>
        </div>
      ) : (
        <div style={{ display: "grid", gridTemplateColumns: desktop ? "repeat(3,1fr)" : "1fr", gap: desktop ? "16px" : "12px" }}>
          {orgEvents.map(ev => (
...
export function OrganizerAlerts() {
  ...
  const alerts = orgEvents.length > 0 ? [
    ...
    { icon: "💰", color: "#16a34a", title: "Revenue Update", body: `Your events have generated GHS ${Math.round(totalRevenue).toLocaleString()} in total revenue at 95% payout rate.`, time: "NOW" },
    ...
    { icon: "🔒", color: "#2563eb", title: "Security: HMAC QR Active", body: "All your event QR codes rotate every 10 seconds. Screenshot sharing is cryptographically prevented.", time: "ALWAYS" },
  ] : [{ icon: "🔔", color: "#f5a623", title: "No alerts yet", body: "Create an event and start selling tickets to see your alerts here.", time: "NOW" }];

  return (
    <div style={{ background: "var(--bg)", minHeight: "100%", padding: desktop ? "28px 40px 60px" : "16px 16px 120px" }}>
      <div style={{ fontSize: "9px", color: "#f5a623", fontWeight: 700, letterSpacing: "2px", marginBottom: "4px", fontFamily: FONT_MONO }}>NOTIFICATIONS</div>
      <h1 style={{ fontWeight: 900, fontSize: desktop ? "26px" : "22px", color: "var(--text-primary)", letterSpacing: "-0.6px", marginBottom: "4px", fontFamily: FONT_DISPLAY }}>Alerts</h1>
      <p style={{ fontSize: "13px", color: "var(--text-muted)", marginBottom: "20px", fontFamily: FONT_MONO }}>BLOCKCHAIN_CONFIRMATIONS · EVENT_ACTIVITY</p>
      <div style={{ maxWidth: desktop ? "640px" : "100%", display: "flex", flexDirection: "column", gap: "10px" }}>
        {alerts.map((a, i) => (
          <motion.div key={i} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.07 }}
            ...
            <div style={{ flex: 1 }}>
              <div style={{ fontWeight: 700, fontSize: "13px", color: "var(--text-primary)", marginBottom: "3px", fontFamily: FONT_DISPLAY }}>{a.title}</div>
              <div style={{ fontSize: "12px", color: "var(--text-secondary)", lineHeight: 1.55, marginBottom: "7px", fontFamily: FONT_BODY }}>{a.body}</div>
              <div style={{ fontSize: "9px", color: a.color, fontWeight: 700, background: a.color + "12", padding: "2px 8px", borderRadius: "99px", width: "fit-content", fontFamily: FONT_MONO }}>{a.time}</div>
            </div>
          </motion.div>
        ))}
      </div>
      <div style={{ marginTop: "20px", padding: "14px 16px", borderRadius: "14px", background: "rgba(124,58,237,0.04)", border: "1px solid rgba(124,58,237,0.12)", display: "flex", alignItems: "center", gap: "12px", maxWidth: desktop ? "640px" : "100%" }}>
        ...
        <div>
          <div style={{ fontSize: "10px", fontWeight: 700, color: "#7c3aed", marginBottom: "2px", fontFamily: FONT_MONO }}>POLYGON_AMOY · CONTRACT: 0x956F...0Daf</div>
          <div style={{ fontSize: "11px", color: "var(--text-muted)", lineHeight: 1.4, fontFamily: FONT_BODY }}>Zero fake tickets. Ownership verified on-chain. Your reputation protected.</div>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div>
          <div style={{ fontSize: "15px", fontWeight: 700, color: "var(--text-primary)", fontFamily: FONT_DISPLAY }}>Create Event</div>
          <div style={{ fontSize: "10px", color: "#7c3aed", marginTop: "1px", fontFamily: FONT_MONO }}>⛓️ NFT_TICKETS_AUTO_MINTED · POLYGON_AMOY</div>
        </div>
      </div>
      ...
      <div style={{
        flex: 1,
        minHeight: 0,          // ← THIS is the scroll fix
        overflowY: "scroll",
        overflowX: "hidden",
        ...
        <div style={{
          ...
          <div style={{ marginBottom: "18px" }}>
            <div style={{ fontSize: "12px", fontWeight: 600, color: "var(--text-secondary)", marginBottom: "9px", fontFamily: FONT_BODY }}>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: "7px" }}>
              {CATEGORIES.map(cat => (
                <motion.div key={cat} whileTap={{ scale: 0.93 }}
                  ...
                  style={{ padding: "6px 14px", borderRadius: "99px", cursor: "pointer", fontSize: "11px", fontWeight: 600, border: "1.5px solid " + (addEventForm.category === cat ? "#f5a623" : errors.category ? "var(--error)" : "var(--border)"), background: addEventForm.category === cat ? "rgba(245,166,35,0.08)" : "var(--bg-card)", color: addEventForm.category === cat ? "#f5a623" : "var(--text-secondary)", transition: "all 0.2s", fontFamily: FONT_BODY }}>
          ...
          {/* Image */}
          <div style={{ marginBottom: "18px" }}>
            <div style={{ fontSize: "12px", fontWeight: 600, color: "var(--text-secondary)", marginBottom: "9px", fontFamily: FONT_BODY }}>Event Image</div>
            <div style={{ display: "flex", gap: "8px", marginBottom: "9px" }}>
              {[["upload","📷 Upload"],["url","🔗 URL"]].map(([t, label]) => (
                <motion.div key={t} whileTap={{ scale: 0.95 }} onClick={() => setImageType(t)}
                  style={{ flex: 1, padding: "8px", borderRadius: "10px", textAlign: "center", cursor: "pointer", fontSize: "12px", fontWeight: 600, border: "1.5px solid " + (imageType === t ? "#f5a623" : "var(--border)"), background: imageType === t ? "rgba(245,166,35,0.08)" : "var(--bg-card)", color: imageType === t ? "#f5a623" : "var(--text-secondary)", transition: "all 0.2s", fontFamily: FONT_BODY }}>
                  {label}
                </motion.div>
              ))}
            </div>
              ...
              <>
                ...
                <label htmlFor="event-image-upload" style={{ display: "block", padding: "20px 16px", background: "var(--bg-card)", border: "2px dashed var(--border)", borderRadius: "12px", textAlign: "center", cursor: "pointer" }}>
                  {addEventForm.image?.startsWith("data:") || addEventForm.image?.startsWith("http") ? (
                    <><img src={addEventForm.image} alt="preview" style={{ width: "100%", height: "140px", objectFit: "cover", borderRadius: "8px", marginBottom: "8px" }} /><div style={{ color: "#16a34a", fontSize: "11px", fontWeight: 700 }}>✅ Image selected · tap to change</div></>
                  ) : (
                    <><div style={{ fontSize: "28px", marginBottom: "8px" }}>📷</div><div style={{ color: "var(--text-secondary)", fontSize: "12px", fontWeight: 600, fontFamily: FONT_BODY }}>Tap to upload JPG or PNG</div><div style={{ color: "var(--text-muted)", fontSize: "10px", marginTop: "2px" }}>Max 5MB</div></>
                  )}
                </label>
              </>
            ) : (
          ...
          <div style={{ display: "grid", gridTemplateColumns: desktop ? "1fr 1fr" : "1fr", gap: desktop ? "0 24px" : "0" }}>
            {fields.map(([key, label, type, placeholder, required]) => (
              <div key={key} style={{ marginBottom: "13px" }}>
                <div style={{ fontSize: "12px", fontWeight: 600, color: "var(--text-secondary)", marginBottom: "6px", fontFamily: FONT_BODY }}>
          ...
          <div style={{ background: "rgba(124,58,237,0.05)", border: "1px solid rgba(124,58,237,0.14)", borderRadius: "11px", padding: "11px 14px", marginBottom: "24px", marginTop: "6px", display: "flex", alignItems: "center", gap: "9px" }}>
            ...
            <span style={{ fontSize: "11px", color: "#7c3aed", fontWeight: 600, fontFamily: FONT_BODY }}>Each ticket sold will be automatically minted as an NFT on Polygon blockchain</span>
          </div>
...
export function OrganizerEventDetail() {
  ...
  if (editing) return (
    <div style={{ background: "var(--bg)", height: "100%", display: "flex", flexDirection: "column", overflow: "hidden", position: "relative" }}>
      ...
      {/* Header */}
      <div style={{ display: "flex", alignItems: "center", padding: "14px 20px", gap: "14px", background: "var(--bg-card)", borderBottom: "1px solid var(--border)", flexShrink: 0, zIndex: 10 }}>
        <motion.button whileTap={{ scale: 0.9 }} onClick={() => setEditing(false)}
          style={{ width: "34px", height: "34px", borderRadius: "9px", background: "var(--bg-subtle)", border: "1px solid var(--border)", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", fontSize: "16px", color: "var(--text-primary)", flexShrink: 0 }}>
          ←
        </motion.button>
        <div>
          <div style={{ fontSize: "15px", fontWeight: 700, color: "var(--text-primary)", fontFamily: FONT_DISPLAY }}>Edit Event</div>
          <div style={{ fontSize: "10px", color: "var(--text-muted)", marginTop: "1px", fontFamily: FONT_MONO }}>EDITING: {ev.name}</div>
        </div>
        {/* Save button in header for quick access */}
        <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}
          onClick={() => { setViewingOrgEvent({ ...ev, ...editForm, price: parseFloat(editForm.price), totalTickets: parseInt(editForm.totalTickets) || ev.totalTickets }); setEditing(false); }}
          style={{ marginLeft: "auto", padding: "8px 18px", background: "linear-gradient(135deg, #f5a623, #e8920f)", color: "#fff", border: "none", borderRadius: "10px", fontSize: "12px", fontWeight: 700, cursor: "pointer", boxShadow: "var(--shadow-brand)", fontFamily: FONT_BODY, flexShrink: 0 }}>
          💾 Save
        </motion.button>
      </div>
      ...
      <div style={{ flex: 1, minHeight: 0, overflowY: "scroll", overflowX: "hidden", WebkitOverflowScrolling: "touch", overscrollBehavior: "contain" }}>
        <div style={{ padding: desktop ? "24px 40px 120px" : "14px 16px 120px", maxWidth: desktop ? "800px" : "100%" }}>

          {/* Category */}
          <div style={{ marginBottom: "18px" }}>
            <div style={{ fontSize: "12px", fontWeight: 600, color: "var(--text-secondary)", marginBottom: "9px", fontFamily: FONT_BODY }}>Category</div>
            <div style={{ display: "flex", flexWrap: "wrap", gap: "7px" }}>
              {CATEGORIES.map(cat => (
                <motion.div key={cat} whileTap={{ scale: 0.93 }}
                  onClick={() => setEditForm(p => ({ ...p, category: cat }))}
                  style={{ padding: "6px 14px", borderRadius: "99px", cursor: "pointer", fontSize: "11px", fontWeight: 600, border: "1.5px solid " + (editForm.category === cat ? "#f5a623" : "var(--border)"), background: editForm.category === cat ? "rgba(245,166,35,0.08)" : "var(--bg-card)", color: editForm.category === cat ? "#f5a623" : "var(--text-secondary)", transition: "all 0.2s", fontFamily: FONT_BODY }}>
                  {cat.charAt(0).toUpperCase() + cat.slice(1)}
                </motion.div>
              ))}
            </div>
            ...
            <div style={{ display: "flex", gap: "8px", marginBottom: "9px" }}>
              {[["upload","📷 Upload"],["url","🔗 URL"]].map(([t, label]) => (
                <motion.div key={t} whileTap={{ scale: 0.95 }} onClick={() => setEditImageType(t)}
                  style={{ flex: 1, padding: "8px", borderRadius: "10px", textAlign: "center", cursor: "pointer", fontSize: "12px", fontWeight: 600, border: "1.5px solid " + (editImageType === t ? "#f5a623" : "var(--border)"), background: editImageType === t ? "rgba(245,166,35,0.08)" : "var(--bg-card)", color: editImageType === t ? "#f5a623" : "var(--text-secondary)", transition: "all 0.2s", fontFamily: FONT_BODY }}>
                  {label}
                </motion.div>
              ))}
            </div>
              ...
              <>
                ...
                <label htmlFor="edit-image-upload" style={{ display: "block", padding: "18px", background: "var(--bg-card)", border: "2px dashed var(--border)", borderRadius: "12px", textAlign: "center", cursor: "pointer" }}>
                  {editForm.image
                    ...
                    : <><div style={{ fontSize: "24px", marginBottom: "6px" }}>📷</div><div style={{ color: "var(--text-secondary)", fontSize: "12px", fontFamily: FONT_BODY }}>Tap to upload</div></>}
                </label>
              </>
            ) : (
          ...
          <div style={{ background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: "12px", padding: "14px 16px", marginBottom: "20px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <div>
              <div style={{ fontSize: "13px", fontWeight: 700, color: "var(--text-primary)", fontFamily: FONT_DISPLAY }}>Ticket Sales</div>
              <div style={{ fontSize: "11px", color: "var(--text-muted)", fontFamily: FONT_BODY, marginTop: "2px" }}>Currently {ev.salesOpen ? "open" : "paused"}</div>
            </div>
            <motion.button whileTap={{ scale: 0.95 }} onClick={() => toggleSales(ev.id)}
              style={{ padding: "8px 16px", background: ev.salesOpen ? "rgba(220,38,38,0.08)" : "rgba(22,163,74,0.08)", color: ev.salesOpen ? "#dc2626" : "#16a34a", border: "1.5px solid " + (ev.salesOpen ? "rgba(220,38,38,0.2)" : "rgba(22,163,74,0.2)"), borderRadius: "10px", fontSize: "12px", fontWeight: 700, cursor: "pointer", fontFamily: FONT_BODY }}>
    ...
    <div style={{ background: "var(--bg)", height: "100%", overflowY: "auto", WebkitOverflowScrolling: "touch", overscrollBehavior: "contain", paddingBottom: desktop ? "60px" : "100px" }}>
      ...
      <div style={{ height: desktop ? "280px" : "220px", position: "relative", flexShrink: 0 }}>
        ...
        <motion.button whileTap={{ scale: 0.95 }} onClick={startEdit}
          style={{ position: "absolute", top: "14px", right: "14px", padding: "6px 14px", background: "rgba(255,255,255,0.15)", backdropFilter: "blur(10px)", border: "1px solid rgba(255,255,255,0.2)", borderRadius: "18px", color: "#fff", fontSize: "11px", fontWeight: 700, cursor: "pointer", fontFamily: FONT_BODY }}>
          ✏️ Edit
        </motion.button>
        ...
        <div style={{ position: "absolute", top: "14px", left: "58px", background: "rgba(124,58,237,0.85)", backdropFilter: "blur(6px)", padding: "3px 9px", borderRadius: "99px", display: "flex", alignItems: "center", gap: "4px" }}>
          ...
          <span style={{ fontSize: "8px", fontWeight: 700, color: "#fff", fontFamily: FONT_MONO }}>NFT_EVENT</span>
        </div>

        <div style={{ position: "absolute", bottom: "18px", left: "20px", right: "20px" }}>
          <div style={{ color: "rgba(255,255,255,0.6)", fontSize: "9px", fontWeight: 700, letterSpacing: "1.5px", marginBottom: "4px", fontFamily: FONT_MONO }}>{(ev.category || "").toUpperCase()}</div>
          <div style={{ color: "#fff", fontWeight: 900, fontSize: desktop ? "26px" : "20px", marginBottom: "4px", fontFamily: FONT_DISPLAY }}>{ev.name}</div>
          <div style={{ color: "rgba(255,255,255,0.6)", fontSize: "11px", fontFamily: FONT_MONO }}>📍 {ev.venue} · {ev.date}</div>
        </div>
      </div>
      ...
      <div style={{ padding: desktop ? "24px 40px" : "14px 16px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: desktop ? "repeat(4,1fr)" : "1fr 1fr", gap: "10px", marginBottom: "14px" }}>
          {[
            ...
            <motion.div key={k} whileHover={{ y: -2 }}
              ...
              <div style={{ fontSize: "16px", fontWeight: 800, color: c, marginBottom: "2px", fontFamily: FONT_DISPLAY }}>{v}</div>
              <div style={{ fontSize: "8px", color: "var(--text-muted)", fontFamily: FONT_MONO }}>{k}</div>
            </motion.div>
          ))}
        </div>
        ...
        <div style={{ background: "var(--bg-card)", borderRadius: "12px", padding: "13px 15px", marginBottom: "12px", border: "1px solid var(--border)" }}>
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "7px" }}>
            <span style={{ fontSize: "12px", fontWeight: 600, color: "var(--text-secondary)", fontFamily: FONT_BODY }}>Ticket Sales</span>
            <span style={{ fontSize: "11px", fontWeight: 700, color: soldPct > 80 ? "#dc2626" : "#16a34a", fontFamily: FONT_MONO }}>{soldPct}%</span>
          </div>
          ...
          <div style={{ fontSize: "10px", color: "var(--text-muted)", marginTop: "4px", fontFamily: FONT_MONO }}>{ev.totalTickets - ev.ticketsSold} REMAINING</div>
        </div>

        {/* Description if any */}
        {ev.description && (
          <div style={{ background: "var(--bg-card)", borderRadius: "12px", padding: "13px 15px", marginBottom: "12px", border: "1px solid var(--border)" }}>
            <div style={{ fontSize: "11px", fontWeight: 700, color: "var(--text-muted)", marginBottom: "6px", fontFamily: FONT_MONO }}>DESCRIPTION</div>
            <div style={{ fontSize: "13px", color: "var(--text-secondary)", lineHeight: 1.65, fontFamily: FONT_BODY }}>{ev.description}</div>
          </div>
        )}
        ...
        <div style={{ background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: "14px", padding: "16px", boxShadow: "var(--shadow-sm)" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "12px" }}>
            ...
            <div>
              <div style={{ fontWeight: 700, fontSize: "13px", color: "var(--text-primary)", fontFamily: FONT_DISPLAY }}>Door Staff Access</div>
              <div style={{ fontSize: "10px", color: "var(--text-muted)", fontFamily: FONT_MONO }}>SINGLE_USE · AUTO_EXPIRE</div>
            </div>
          </div>

          <motion.button whileHover={{ borderColor: "#f5a623" }} whileTap={{ scale: 0.97 }}
            onClick={() => generateDoorCode(ev.id, ev.name)}
            style={{ width: "100%", padding: "10px", background: "rgba(245,166,35,0.06)", color: "#f5a623", border: "2px dashed rgba(245,166,35,0.3)", borderRadius: "10px", fontSize: "12px", fontWeight: 700, cursor: "pointer", marginBottom: "10px", transition: "border-color 0.2s", fontFamily: FONT_BODY }}>
          ...
          {invites.map(inv => (
            <motion.div key={inv.code} whileTap={{ scale: 0.98 }} onClick={() => copyCode(inv.code)}
              ...
              <span style={{ fontFamily: FONT_MONO, fontWeight: 700, color: inv.used ? "var(--text-muted)" : "#f5a623", fontSize: "13px", letterSpacing: "1px" }}>{inv.code}</span>
              <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                <span style={{ fontSize: "9px", color: inv.used ? "var(--text-muted)" : "#16a34a", fontWeight: 700, fontFamily: FONT_MONO }}>{inv.used ? "USED" : "ACTIVE"}</span>
                {!inv.used && <span style={{ fontSize: "9px", color: copiedCode === inv.code ? "#16a34a" : "var(--text-muted)", fontFamily: FONT_MONO }}>{copiedCode === inv.code ? "COPIED!" : "TAP_COPY"}</span>}
              </div>
            </motion.div>
          ))}
          ...
          <div style={{ padding: "9px 11px", borderRadius: "9px", background: "rgba(37,99,235,0.05)", border: "1px solid rgba(37,99,235,0.12)", marginTop: "4px" }}>
            <div style={{ fontSize: "10px", color: "#2563eb", fontWeight: 600, fontFamily: FONT_BODY }}>🔒 Door staff can only scan — cannot manage events</div>
          </div>
        </div>
      </div>
    </div>
