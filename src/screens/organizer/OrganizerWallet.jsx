...
const btnStyle = {
  ...
  boxShadow: "0 8px 24px rgba(245,166,35,0.28)", marginBottom: "0",
};
...
export default function OrganizerWallet() {
  ...
  return (
    <div style={{ background: "#f8f8f6", minHeight: "100%", padding: desktop ? "40px" : "20px 20px 100px" }}>
      <div style={{ maxWidth: desktop ? "900px" : "100%", margin: "0 auto" }}>

        {/* Header */}
        {desktop && (
          <div style={{ marginBottom: "28px" }}>
            <div style={{ fontSize: "12px", color: "#f5a623", fontWeight: 700, letterSpacing: "1px", marginBottom: "4px" }}>WALLET</div>
            <div style={{ fontWeight: 800, fontSize: "28px", color: "#1a1a1a", letterSpacing: "-0.5px" }}>Your Earnings</div>
            <div style={{ color: "#aaa", fontSize: "14px", marginTop: "4px" }}>Track your revenue and withdraw funds</div>
          </div>
        )}
        ...
        {loading ? (
          ...
          <div style={{ display: "grid", gridTemplateColumns: desktop ? "1fr 1fr" : "1fr", gap: desktop ? "24px" : "0" }}>
            ...
            <div>
              {/* Balance card */}
              <div style={{ background: "linear-gradient(135deg, #f5a623, #e8920f)", borderRadius: "24px", padding: "28px", color: "#fff", boxShadow: "0 8px 40px rgba(245,166,35,0.3)", marginBottom: "20px" }}>
                <div style={{ fontSize: "12px", fontWeight: 700, opacity: 0.85, letterSpacing: "1px", marginBottom: "8px" }}>AVAILABLE BALANCE</div>
                <div style={{ fontSize: "44px", fontWeight: 900, marginBottom: "4px", letterSpacing: "-1px" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}>
                {[
                  ["💰", "Total Earned",  "Ghc " + Math.round(totalEarned).toLocaleString(),    "#27ae60"],
                  ["💸", "Withdrawn",     "Ghc " + Math.round(totalWithdrawn).toLocaleString(), "#5dade2"],
                  ["🔗", "Fees Paid",     "Ghc " + Math.round(feesPaid).toLocaleString(),       "#e74c3c"],
                  ["📊", "Balance",       "Ghc " + Math.round(balance).toLocaleString(),         "#f5a623"],
                ].map(([icon, label, value, color]) => (
                  <div key={label} className="stat-card" style={{ background: "#fff", borderRadius: "16px", padding: "16px", border: "1px solid #f0f0f0", boxShadow: "0 2px 12px rgba(0,0,0,0.05)" }}>
      ...
      {showModal && (
        <>
          ...
          <div style={{ position: "fixed", bottom: 0, left: "50%", transform: "translateX(-50%)", width: desktop ? "480px" : "100%", background: "#fff", borderRadius: "28px 28px 0 0", zIndex: 201, padding: "28px 28px 48px", boxShadow: "0 -8px 40px rgba(0,0,0,0.12)" }}>
            ...
            {step === 2 && (
              <>
                ...
                <div style={{ marginTop: "20px", display: "flex", gap: "10px" }}>
                  ...
                  <button onClick={handleWithdraw} style={{ ...btnStyle, flex: 2 }}>Confirm</button>
                </div>
              </>
            )}
            ...
            {step === 3 && (
              <div style={{ textAlign: "center" }}>
                ...
                <button onClick={() => { setShowModal(false); setStep(1); setAmount(""); setMomoNumber(""); }} style={btnStyle}>Done</button>
              </div>
            )}
          </div>
        </>
